...
function handleRequest(e) {
  try {
    ...
    var publicActions = [
      "getPublicStats","getPlans","getPlan","validateDiscount","getWalletAddress",
      "initUser","getUser","markOnboarded","submitOrder","adminLogin",
    ...
    if (action === "getWalletAddress") return getWalletAddress();
    ...
    if (action === "markOnboarded") return markOnboarded(params);
...
function initUser(params) {
  ...
  if (params.referredBy) {
    ...
    var referrer = sheetToObjects(refSheet).find(function(u){ return String(u.referralCode) === String(params.referredBy); });
    if (referrer) {
      var refId = genId("ref");
...
function getUserAccounts(params) {
  ...
  accounts = accounts.map(function(a){
    ...
    a.planAccountSize = plan.accountSize || "";
    a.planPrice = plan.price || "";
    a.planPhase1Target = plan.phase1Target || "";
    a.planPhase2Target = plan.phase2Target || "";
    a.planMaxDrawdown = plan.maxDrawdown || "";
    a.planPayoutSplit1 = plan.payoutSplit1 || "";
    a.planPayoutSplitN = plan.payoutSplitN || "";
    a.planDetails = plan;
    a.payouts = payouts.filter(function(p){ return p.accountId === a.accountId; });
    return a;
  });
  return ok(accounts);
}
...
function getReferrals(params) {
  ...
  refs = refs.map(function(r){
    ...
    r.refereeName = (referee.firstName || "") + (referee.lastName ? " " + referee.lastName : "") || referee.username || String(r.refereeTelegramId);
    r.refereeUsername = referee.username || "";
    r.refereeId = r.refereeTelegramId;
    return r;
  });
  return ok(refs);
}
...
function getSettingsAll(params) {
  ...
  var safe = {};
  var allowed = ["walletAddress","payoutSplit1","payoutSplitN","referralPoints","blownRefundEnabled","blownRefundPct","appUrl","adminChatId","botUsername"];
...
function saveSettingAction(params) {
  ...
  var allowed = ["walletAddress","payoutSplit1","payoutSplitN","referralPoints","blownRefundEnabled","blownRefundPct","appUrl","adminChatId","adminPassword","botToken","botUsername"];
...
function getWalletAddress() {
  return ok({ walletAddress: getSetting("walletAddress"), botUsername: getSetting("botUsername") });
}
